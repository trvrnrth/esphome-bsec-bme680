...
void BME680BSECComponent::setup() {
  ...
  this->bsec_.begin(
    this->address_,
    BME680_I2C_INTF,
    BME680BSECComponent::read_bytes_wrapper,
    BME680BSECComponent::write_bytes_wrapper,
    Bsec::delay_ms
  );
...
void BME680BSECComponent::dump_config() {
  ...
  ESP_LOGCONFIG(
    ...
    this->bsec_.version.major, this->bsec_.version.minor,
  ...
  if (this->is_failed()) {
    ESP_LOGE(
      TAG, "Communication failed with BSEC Status: %d, BME680 Status: %d",
      this->last_bsec_status_, this->last_bme680_status_
    );
  }
...
void BME680BSECComponent::set_iaq_mode(BME680BSECIAQMode iaq_mode) {
  this->iaq_mode_ = iaq_mode;
}

void BME680BSECComponent::set_state_save_interval(uint16_t interval) {
  this->state_save_interval_ = interval;
}

uint8_t BME680BSECComponent::get_iaq_(void) {
  return this->iaq_mode_ == BME680_IAQ_MODE_STATIC ? this->bsec_.staticIaq : this->bsec_.iaq;
}

uint8_t BME680BSECComponent::get_iaq_accuracy_(void) {
...
bool BME680BSECComponent::check_bsec_status_(void) {
  ...
  if (this->last_bsec_status_ < BSEC_OK || this->last_bme680_status_ < BME680_OK) {
    return false;
  }
...
void BME680BSECComponent::load_state_(void) {
